...
def draw_genome(edges, genome, size):
    im = Image.new(mode='RGB', size=size, color=(192, 192, 192))
    d = ImageDraw.Draw(im)
    for i in range(len(genome)-2):
        d.line(xy=(edges[genome[i][0]], edges[genome[i+1][0]]), fill=(0, 0, 0, 50), width=1)
...
def process(n_steps, edges, genome, imt, mask):
    ...
    for i in range(n_steps):
        ...
        new_genome[random.randint(0, len(genome)-1)][0] = random.randint(0, len(edges)-1)
        new_genome[random.randint(0, len(genome)-1)][1] = random.randint(0, len(edges)-1)
        img = draw_genome(edges, new_genome, imt.size)
        pg = list(img.getdata())
...
def main(imt, edges, n_steps, genome_file_name=None, genome_size=None, mask_file_name=None):
    if genome_file_name:
        ...
        if genome_size and genome_size > 0:
            genome = [[random.randint(0, len(edges)-1), random.randint(0, len(edges)-1)] for i in range(genome_size)]      
